...
function push(_in_){
	node = malloc();
	node.data = _in_;
	if(!CAS(ToS, NULL, node)){  *** IF(ToS === NULL) in(node.data) ***
		free(node);
	}
}

function pop(){
	node = ToS;  *** IF(ToS == NULL) out(/) ***
	if(node == NULL){
		return;
	}
	if(CAS(ToS, node, NULL)){  *** IF(ToS === node) out(node.data) ***
		_out_ = node.data;
		free(node);
	}
}
...
Program *testprogram(){
	...
	Function *push = new Function(
		...
		new Sequence(std::vector<Block*>({
			...
			new IfThen(
				new CAS(tos, null, node, true, new LinearisationEvent(IN_PCMP, node, tos, null)),
				new Sequence(std::vector<Block*>({
					new Free(node)
				}))
			)
	...
	Function *pop = new Function(
		...
		false,
		new Sequence(std::vector<Block*>({
			new PointerAssignment(node, tos, new LinearisationEvent(OUT_PCMP, NULL, tos, null)),
			new IfThen(
				new PointerComparison(node, null, false),
				new Sequence(std::vector<Block*>({
					new Return()
				}))
			),
			new IfThen(
				new CAS(tos, node, null, false, new LinearisationEvent(OUT_PCMP, node, tos, node)),
				new Sequence(std::vector<Block*>({
					new OutputAssignment(node),
					new Free(node)
				}))
			)
		}))
	...
	Program *one = new Program(
		"onedate",
		std::vector<Variable*>({null, tos, node}),
		init,
...
Observer *testobserver(){
	return onevalobserver();
}


void OneThreadView::build_initial(){
	int tosid = 1, nullid = 3;
